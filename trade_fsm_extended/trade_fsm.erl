...
-export([init/1, handle_event/3, handle_sync_event/4, handle_info/3,
         ...
         negotiate/3, receive_offer/2, receive_undo_offer/2, make_own_offer/2, undo_own_offer/2, wait/2, ready/2, ready/3]).
...
negotiate({make_offer, Item}, S=#state{ownitems=OwnItems}) ->
    ...
    {next_state, receive_offer, S#state{ownitems=add(Item, OwnItems)}};
%% Own side retracting an item offer
negotiate({retract_offer, Item}, S=#state{ownitems=OwnItems}) ->
    undo_offer(S#state.other, Item),
    notice(S, "cancelling offer on ~p", [Item]),
    {next_state, receive_undo_offer, S#state{ownitems=remove(Item, OwnItems)}};
%% other side offering an item
negotiate({do_offer, Item}, S=#state{otheritems=OtherItems}) ->
    notice(S, "other player offering ~p", [Item]),
    {next_state, make_own_offer, S#state{otheritems=add(Item, OtherItems)}};
%% other side retracting an item offer
negotiate({undo_offer, Item}, S=#state{otheritems=OtherItems}) ->
    notice(S, "Other player cancelling offer on ~p", [Item]),
    {next_state, undo_own_offer, S#state{otheritems=remove(Item, OtherItems)}};
...
negotiate(Event, _From, S) ->
    ...
    {next_state, negotiate, S}.
%% other side offering an item
receive_offer({do_offer, Item}, S=#state{otheritems=OtherItems}) ->
    notice(S, "other player offering ~p", [Item]),
    {next_state, negotiate, S#state{otheritems=add(Item, OtherItems)}};
receive_offer(Event, Data) ->
    unexpected(Event, recive_offer),
    {next_state, receive_offer, Data}.
%% other side retracting an item offer
receive_undo_offer({undo_offer, Item}, S=#state{otheritems=OtherItems}) ->
    notice(S, "Other player cancelling offer on ~p", [Item]),
    {next_state, negotiate, S#state{otheritems=remove(Item, OtherItems)}};
receive_undo_offer(Event, Data) ->
    unexpected(Event, recive_undo_offer),
    {next_state, receive_undo_offer, Data}.
make_own_offer({make_offer, Item}, S=#state{ownitems=OwnItems}) ->
    do_offer(S#state.other, Item),
    notice(S, "offering ~p", [Item]),
    {next_state, negotiate, S#state{ownitems=add(Item, OwnItems)}};
make_own_offer(Event, Data) ->
    unexpected(Event, make_own_offer),
    {next_state, make_own_offer, Data}.
%% other side retracting an item offer
undo_own_offer({retract_offer, Item}, S=#state{ownitems=OwnItems}) ->
    undo_offer(S#state.other, Item),
    notice(S, "cancelling offer on ~p", [Item]),
    {next_state, negotiate, S#state{ownitems=remove(Item, OwnItems)}};
undo_own_offer(Event, Data) ->
    unexpected(Event, undo_own_offer),
    {next_state, undo_own_offer, Data}.
